...
class BatchGenerator:
    ...
    def __init__(self, dataset: Dataset, num: int, shuffle: bool, op: Op = None):
        ...
        if not isinstance(dataset, Dataset):
            raise TypeError(
                "The dataset should be an instance of Dataset class")
        if not isinstance(num, int) or num <= 0:
            raise TypeError(
                "The number of samples per batch should be a positive integer")
        if num > len(dataset):
            raise TypeError(
                "The number of samples per batch should not be bigger than the length of dataset")
        if not isinstance(shuffle, bool):
            raise TypeError("The shuffle argument should be boolean")

        if num > len(dataset):
            raise ValueError(
                "The number of samples per batch should be less than or equal to the dataset size")

        data = dataset.data
        if op:
            data = op(dataset.data).reshape(dataset.__len__(), 32, 32, 3)

        indexed_elements = [(index, element, label) for index,
                            (element, label) in enumerate(zip(data, dataset.labels))]

        if shuffle:
            np.random.shuffle(indexed_elements)

        chunks = [indexed_elements[i:i + num]
                  for i in range(0, len(indexed_elements), num)]
        ...
        for chunk in chunks:
            ...
            batch = Batch(np.stack(data, axis=0), np.stack(
                labels, axis=0), np.stack(indices, axis=0))
